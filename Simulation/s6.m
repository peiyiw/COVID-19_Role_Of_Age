%Seed & Age-structure & Contact matrix
...
% *******************Seed*******************
Lstruct_homo = repmat(1/7, 1, 7);
HL(1,1:n) = theta(3) .* Lstruct_homo; %latent
HIa(1,1:n) = theta(4) .* Lstruct_homo; %infectious asymptomatic
HLp(1,1:n) = theta(5) .* Lstruct_homo; %infectious presymptomatic
HIs(1,1:n)=theta(6) .* Lstruct_homo; %infectious symptomatic
% *******************************************
Is(1,1:n)=0; 
Ia(1,1:n)=0;
...
for i = 2:duration %time period
    ...
    % *******************Contact matrix*******************
    cm = repmat(sum(contactmatrix, 'all')/49,7,7);
    beta = cm .* beta_b';
    % ****************************************************
    beta_a = r .* beta;
    beta_p = k .* beta;
